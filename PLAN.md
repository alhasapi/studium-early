...
## Current state
- Phase 1 is largely done. `edu_core` holds a validating content registry
  (`content.lua`), a small reviewed `early_en` pack, one shared task selector
  (`content.pick`), task-block publishing (`edu.set_task_blocks`), and per-player
  per-skill progress (`edu.record_result`).
- Every activity reads its tasks from content and falls back to built-in material
  when a pack supplies none: 26 words for English, three patterns for logic, and
  generated addition/subtraction within 10 for arithmetic.
- `accepted_nodes` and `distractor_nodes` drive the child's block palette for the
  current task, so a pack's curated distractors actually reach the child.
- Strings are marked with `S()` and templates are generated by
  `tools/update-locale.sh`; no translated `.po` file exists yet.
- Still open: adaptive selection (Phase 2), the counting/comparison/number-bond
  and phonics/sentence task types (Phases 2–3), the remaining logic task types
  (Phase 4), and the spreadsheet/JSON authoring workflow (Phase 5).
